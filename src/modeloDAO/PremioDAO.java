...
public class PremioDAO {
	...
	public List<Factura> elegirFacturasParaObsequio(int idCliente, Connection conexionExitosa) {
		...
		consultaSQL = "select * from mercadouq.factura  where  fecha_factura >= DATE_SUB(CURRENT_DATE, INTERVAL 24 MONTH) and id_cliente ="
	...
	private int obtenerLimitConsulta(int idCliente, Connection conexionExitosa) {
		...
		PreparedStatement sentenciaPreparada = null;
		ResultSet resultadoConsulta = null;
		String consultaSQL = "select CEIL(COUNT(*) * 0.1)  from mercadouq.factura where  fecha_factura >= DATE_SUB(CURRENT_DATE, INTERVAL 24 MONTH) and id_cliente = "
				+ idCliente;
